...
def ENet_weighting(image_files=image_files, num_classes=12):
    """
    The custom class weighting function as seen in the ENet paper.
    INPUTS:
    - image_files(list): a list of image_filenames which element can be read immediately
    OUTPUTS:
    - class_weights(list): a list of class weights where each index represents each class label and the element is the
                            class weight for that label.
    """
    ...
    # Perform the weighting function label-wise and append the label's class weights to class_weights
    class_weights = []
...
def MFB_weighting(image_files=image_files, num_classes=12):
    """
    ...
    for n in range(len(image_files)):
        ...
        # For each image sum up the frequency of each label in that image and append to the dictionary if
...
if __name__ == "__main__":
    MFB_weighting(image_files, num_classes=12)
