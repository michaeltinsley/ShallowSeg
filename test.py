...
flags.DEFINE_string('logdir', './log/original_test',
                    'The log directory for event files created during test evaluation.')
...
# Dataset directories
image_files = sorted(
    [os.path.join(dataset_dir, 'test', file) for file in os.listdir(dataset_dir + "/test") if file.endswith('.png')])
annotation_files = sorted(
    [os.path.join(dataset_dir, "testannot", file) for file in os.listdir(dataset_dir + "/testannot") if
     file.endswith('.png')])
...
def run():
    with tf.Graph().as_default() as graph:
        ...
        with slim.arg_scope(ShallowSeg_arg_scope()):
            logits, probabilities = ShallowSeg(inputs=images,
